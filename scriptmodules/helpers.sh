...
## @file helpers.sh
## @brief RetroArena helpers library
## @copyright GPLv3
...
function getDepends() {
    ...
    for required in $@; do
        ...
        if [[ "$required" == "libpng12-dev" ]] && compareVersions "$__os_debian_ver" ge 9;  then
            required="libpng-dev"
            printMsgs "console" "RetroArena module references libpng12-dev and should be changed to libpng-dev"
        fi
        ...
        if [[ "$md_mode" == "install" ]]; then
            ...
            #if [[ "$own_ffmpeg" -eq 1 && "$required" == "ffmpeg" ]]; then
                packages+=("$required")
                continue
            fi
...
# @fn setupDirectories()
# @brief Makes sure some required retroarena directories and files are created.
function setupDirectories() {
    ...
    if [[ ! -f "$config" ]]; then
        echo "# this file can be used to enable/disable retroarena autoconfiguration features" >"$config"
    fi
...
## @brief Creates a default retroarch.cfg for specified system in `/opt/retroarena/configs/$system/retroarch.cfg`.
...
# added by RetroArena-Setup - $res mode for emulators
mode "$res"
...
function getPlatformConfig() {
    ...
    # workaround for RetroArena platform
    [[ "$key" == "retroarena_fullname" ]] && ini_value="RetroArena"
    echo "$ini_value"
}
