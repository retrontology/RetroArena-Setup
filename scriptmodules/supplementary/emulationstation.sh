...
rp_module_desc="EmulationStation - Frontend used by RetroArena for launching emulators"
...
function depends_emulationstation() {
    local depends=(
        ...
        libcurl4-openssl-dev libasound2-dev cmake libsm-dev libsdl2-dev
...
function remove_emulationstation() {
    ...
    if isPlatform "x11"; then
        rm -rfv "/usr/local/share/icons/retroarena.svg" "/usr/local/share/applications/retroarena.desktop"
    fi
}
